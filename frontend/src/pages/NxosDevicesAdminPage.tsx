...
import {
  ...
  testNxosDevice,
  updateNxosDevice,
  type NxosConnectivityResult,
  type NxosDeviceUpdate
} from "@/services/nxos";
...
export function NxosDevicesAdminPage() {
  ...
  const [testResults, setTestResults] = useState<Record<string, NxosConnectivityResult>>({});
  const [testErrors, setTestErrors] = useState<Record<string, string>>({});
  const [editingDevice, setEditingDevice] = useState<NxosDevice | null>(null);
  const [editForm, setEditForm] = useState<NxosDeviceUpdate>({});
  ...
  const handleTest = async (id: string) => {
    ...
    try {
      const result = await testNxosDevice(id);
  ...
  const openEdit = (device: NxosDevice) => {
  ...
  const handleUpdate = async (e: FormEvent) => {
    ...
    setIsUpdating(true);
    setUpdateError(null);
    const payload: NxosDeviceUpdate = { ...editForm };
    ...
    try {
      const updated = await updateNxosDevice(editingDevice.id, payload);
                  ...
                  <form className="mt-4 space-y-4" onSubmit={handleUpdate}>
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input className={field} value={editForm.role ?? ""} placeholder="Nexus" onChange={(e) => setEditForm({ ...editForm, role: e.target.value })} />
                      </div>
                      <div>
                        <label className="mb-1 block text-xs uppercase tracking-wide text-slate-400">Username</label>
