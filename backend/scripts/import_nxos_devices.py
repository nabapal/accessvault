#!/usr/bin/env python3
"""Import Cisco NX-OS (Nexus) devices into the inventory from Nautobot, by role.

For every Nautobot device whose role matches one of the requested roles (default:
"Nexus") and which has a primary management IP, this upserts an nxos_devices row
(keyed by mgmt IP), pre-populating role / site / rack-location from Nautobot. SSH
credentials are taken from --username/--password (or NET_USERNAME/NET_PASSWORD in the
environment). With --collect it also SSH-collects reachable devices immediately;
otherwise the normal background poller will collect them.

Examples:
  python scripts/import_nxos_devices.py                       # role Nexus, active
  python scripts/import_nxos_devices.py --role Nexus --collect --dry-run
"""
...
from app.models import NxosDevice, NxosPlatform  # noqa: E402
from app.services.crypto import encrypt_secret  # noqa: E402
from app.services.nxos_collector import run_collection_for_device  # noqa: E402
from app.services.nautobot import compute_device_facts  # noqa: E402


def parse_args(argv: List[str]) -> argparse.Namespace:
    settings = get_settings()
    parser = argparse.ArgumentParser(description="Bulk-import NX-OS devices from Nautobot by role.")
    parser.add_argument("--role", action="append", default=[], metavar="ROLE",
                        help="Nautobot device role to import (repeatable, default: Nexus)")
    ...
    missing = [n for n, v in (("--nautobot-url", args.nautobot_url),
...
async def run(args: argparse.Namespace) -> int:
    ...
    async with AsyncSessionLocal() as session:
        for device in devices:
            ...
            platform = NxosPlatform.from_raw(_node_name(device.get("platform")))
            ...
            existing = (
                await session.execute(select(NxosDevice).where(NxosDevice.mgmt_ip == mgmt_ip))
            ).scalar_one_or_none()
            device_row = existing or NxosDevice(mgmt_ip=mgmt_ip)
        ...
        if args.collect and not args.dry_run:
            rows = (await session.execute(select(NxosDevice).where(NxosDevice.mgmt_ip.in_(to_collect)))).scalars().all()
            for row in rows:
    ...
    if args.collect and not args.dry_run:
        ...
        print("Devices will be collected by the background NX-OS poller.")
    return 0
